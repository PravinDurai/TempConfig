...
<xsl:variable name="source">
 ...
 <xsl:variable name="originator">
    <xsl:value-of select="$context/c:context/c:entry[@key='asg:httpRequest']/h:request/h:header[@name='x-originator']"/>
</xsl:variable>
...
<xsl:variable name="NotifyEventCode">
  <xsl:copy-of select="$context/c:context/c:entry[@key='asg:httpRequest']/h:request/h:header[@name='x-notify-eventcode']"/>
</xsl:variable>
...
<xsl:variable name="GetOriginatorName">
	<xsl:choose>
		<xsl:when test="$context/c:context/c:entry[@key='asg:httpRequest']/h:request/h:header[@name='x-originator'] != '' ">
			<xsl:value-of select="$originator"/></xsl:when>
		<xsl:otherwise><xsl:value-of select="$GetPartnerName"/></xsl:otherwise>
	</xsl:choose>
</xsl:variable>
	  ...
	  <map:mapping-result>
	   <map:context>
		<c:context>
			...
			<k:override-header name="originator"><xsl:value-of select="$GetOriginatorName"/></k:override-header>
			...
			<k:override-header name="status"><xsl:value-of select="$NotifyEventCode"/></k:override-header>
           </c:entry>
		</c:context>
	   </map:context>

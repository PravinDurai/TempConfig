	...
	<xsl:template match="/">
		<SOAP-ENV:Envelope>
				...
				<xsl:choose>
					<xsl:when test="$sbResponse/er-response[@id='100001']">
						<SOAP-ENV:Fault>
							...
							<faultactor/>
							<detail>
								<ns0:ModifyUserGroupserviceException xmlns:ns0 = "http://www.vodafone.com/vf/contentSellingManagement/interface/v.1.1">
            ...
            , error@system-id - <xsl:value-of select="$sbResponse/er-response/payload/error/attribute::system-id"/>
								...
								</ns0:ModifyUserGroupserviceException>
							</detail>
						</SOAP-ENV:Fault>
					</xsl:when>
					<xsl:otherwise>
						<v:ModifyUserGroupResponse>
							<v1:er-response>
								...
								<v1:modify-usergroup-response>
									<v1:success>
										<xsl:value-of select="$sbResponse/er-response/payload/modify-usergroup-reponse/success"/>
									</v1:success>
									<v1:reason-code>
										<code>
											<xsl:value-of select="$sbResponse/er-response/payload/modify-usergroup-reponse/reason-code/code"/>
										</code>
										<xsl:if test="$sbResponse/er-response/payload/modify-usergroup-reponse/reason-code/sub-code">
											<sub-code>
												<xsl:value-of select="$sbResponse/er-response/payload/modify-usergroup-reponse/reason-code/sub-code"/>
											</sub-code>
										</xsl:if> 
										<name>
											<xsl:value-of select="$sbResponse/er-response/payload/modify-usergroup-reponse/reason-code/name"/>
										</name>
									</v1:reason-code> 
								</v1:modify-usergroup-response>
							</v1:er-response>
							<v1:traceIdentifier>
								<v2:applicationID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:ModifyUserGroup/v1:traceIdentifier/v2:applicationID"/>
								</v2:applicationID>
								<v2:serviceID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:ModifyUserGroup/v1:traceIdentifier/v2:serviceID"/>
								</v2:serviceID>
								<v2:serviceInterfaceVersion>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:ModifyUserGroup/v1:traceIdentifier/v2:serviceInterfaceVersion"/>
								</v2:serviceInterfaceVersion>
								<v2:timestamp>
									<xsl:value-of select="current-dateTime()"/>
								</v2:timestamp>
								<v2:referenceID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:ModifyUserGroup/v1:traceIdentifier/v2:referenceID"/>
								</v2:referenceID>
								<v2:correlationID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:ModifyUserGroup/v1:traceIdentifier/v2:correlationID"/>
								</v2:correlationID>
								<v2:identityID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:ModifyUserGroup/v1:traceIdentifier/v2:identityID"/>
								</v2:identityID>
							</v1:traceIdentifier>
						</v:ModifyUserGroupResponse>
					</xsl:otherwise>

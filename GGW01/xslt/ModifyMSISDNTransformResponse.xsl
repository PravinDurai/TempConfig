...
<xsl:stylesheet version="1.0" 	xmlns:ns1="http://www.vodafone.com/rig/internal/validateheader/v1.0"	xmlns:xsl="http://www.w3.org/1999/XSL/Transform"	xmlns:SOAP-ENV="http://www.w3.org/2003/05/soap-envelope"	xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"	xmlns:head="http://xmlns.vodafone.co.uk/map/til/wsdl/Common/Header"	xmlns:log="http://xmlns.vodafone.co.uk/map/til/Common/CommonObjects/Logon"	xmlns:v="http://www.vodafone.com/vf/contentSellingManagement/interface/v.1.1"	xmlns:v1="http://www.vodafone.com/vf/contentSellingManagement/messages/v.1.1"	xmlns:v2="http://www.vodafone.com/vf/core_common/types/v.0.3/" xmlns:asg_map="http://www.tibco.com/asg/mapping" xmlns:err="http://www.tibco.com/schemas/asg/error" xmlns:c="http://www.tibco.com/schemas/asg/context" xmlns:h="http://www.tibco.com/asg/protocols/http" xmlns:asg="http://www.tibco.com/asg/error">
	...
	<xsl:variable name="IdentityID"><xsl:value-of select="$context/v:ModifyMSISDN/v1:traceIdentifier/v2:identityID"/></xsl:variable>
	...
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="$IdentityID='VFUKTIL'">
				<xsl:choose>
					<xsl:when test="$sbResponse/er-response[@id='100001']">
						<v:ModifyMSISDNServiceFault  xmlns:ns1 = "http://xmlns.vodafone.co.uk/RIG/common/Fault">
            ...
            , error@system-id - <xsl:value-of select="$sbResponse/er-response/payload/error/attribute::system-id"/>
						...
						</v:ModifyMSISDNServiceFault>
					</xsl:when>
					<xsl:otherwise> 
						<v:ModifyMSISDNResponse>
							<v1:er-response>
								...
								<v1:modify-msisdn-response>
									<v1:success>
										<xsl:value-of select="$sbResponse/er-response/payload/modify-msisdn-response/success"/>
									</v1:success>
								</v1:modify-msisdn-response>
							</v1:er-response>
							<v1:traceIdentifier>
								<v2:applicationID>
									<xsl:value-of select="$context/v:ModifyMSISDN/v1:traceIdentifier/v2:applicationID"/>
								</v2:applicationID>
								<v2:serviceID>
									<xsl:value-of select="$context/v:ModifyMSISDN/v1:traceIdentifier/v2:serviceID"/>
								</v2:serviceID>
								<v2:serviceInterfaceVersion>
									<xsl:value-of select="$context/v:ModifyMSISDN/v1:traceIdentifier/v2:serviceInterfaceVersion"/>
								</v2:serviceInterfaceVersion>
								<v2:timestamp>
									<xsl:value-of select="current-dateTime()"/>
								</v2:timestamp>
								<v2:referenceID>
									<xsl:value-of select="$context/v:ModifyMSISDN/v1:traceIdentifier/v2:referenceID"/>
								</v2:referenceID>
								<v2:correlationID>
									<xsl:value-of select="$context/v:ModifyMSISDN/v1:traceIdentifier/v2:correlationID"/>
								</v2:correlationID>
								<v2:identityID>
									<xsl:value-of select="$context/v:ModifyMSISDN/v1:traceIdentifier/v2:identityID"/>
								</v2:identityID>
							</v1:traceIdentifier>
						</v:ModifyMSISDNResponse>
					</xsl:otherwise>

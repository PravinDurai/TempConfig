...
<xsl:variable name="httpPayload">
  <xsl:copy-of select="codecs:base64ToText($context/c:context/c:entry[@key='asg:httpRequest']/h:request/h:body)"/>
</xsl:variable>
	 ...
	 <map:context>
	  <c:context>
	   <c:entry key="asg:jmsRequest">
            ...
            <k:override-header name="security_token"><xsl:value-of select="$securityToken" /></k:override-header>

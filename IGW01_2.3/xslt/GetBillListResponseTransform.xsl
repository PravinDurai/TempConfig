...
<xsl:stylesheet version="1.0" xmlns:ns1="http://www.vodafone.com/rig/internal/validateheader/v1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:req="http://xmlns.vodafone.co.uk/map/til/wsdl/Common/RequestHeader" xmlns:head="http://xmlns.vodafone.co.uk/map/til/wsdl/Common/Header" xmlns:log="http://xmlns.vodafone.co.uk/map/til/Common/CommonObjects/Logon" xmlns:ns0="http://xmlns.vodafone.co.uk/map/til/Billing/CustomerBillProduction/GetBillList" xmlns:asg_map="http://www.tibco.com/asg/mapping" xmlns:err="http://www.tibco.com/schemas/asg/error" xmlns:c="http://www.tibco.com/schemas/asg/context" xmlns:h="http://www.tibco.com/asg/protocols/http" xmlns:asg="http://www.tibco.com/asg/error" xmlns:ns="http://xmlns.vodafone.co.uk/map/til/Common/ResultStatus">
	...
	<xsl:variable name="Severity">
		<xsl:value-of select="$sbResponse/ns0:GetBillListResponse/ns:ResultStatus/ns:severity"/>
	</xsl:variable>
	<xsl:variable name="ErrorCode">
		<xsl:value-of select="$sbResponse/ns0:GetBillListResponse/ns:ResultStatus/ns:code"/>
	</xsl:variable>
	<xsl:variable name="Message">
		<xsl:value-of select="$sbResponse/ns0:GetBillListResponse/ns:ResultStatus/ns:message"/>
	</xsl:variable>
	<xsl:variable name="Component">
		<xsl:choose>
			<xsl:when test="string-length($sbResponse/ns0:GetBillListResponse/ns:ResultStatus/ns:component) &gt; '0'">
				<xsl:value-of select="$sbResponse/ns0:GetBillListResponse/ns:ResultStatus/ns:component"/>
			</xsl:when>
			<xsl:otherwise>BW</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="Operation">
		<xsl:value-of select="$sbResponse/ns0:GetBillListResponse/ns:ResultStatus/ns:operation"/>
	</xsl:variable>
	...
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="$sbResponse/ns0:GetBillListResponse/ns:ResultStatus/ns:severity='S'">
				<SOAP-ENV:Envelope>
					<SOAP-ENV:Header>
					...
					<SOAP-ENV:Body>
						<ns0:GetBillListResponse xmlns = "http://xmlns.vodafone.co.uk/map/til/Billing/CustomerBillProduction/GetBillList">
							<xsl:copy-of select="$sbResponse/ns0:GetBillListResponse/*"/>
						</ns0:GetBillListResponse>
					</SOAP-ENV:Body>
				</SOAP-ENV:Envelope>
			</xsl:when>
				...
				<SOAP-ENV:Envelope xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/">
					<SOAP-ENV:Body>
						<SOAP-ENV:Fault>
							...
							<detail>
								<v:GetBillListserviceException xmlns:v="http://www.vodafone.com/vf/customerInterfaceManagement/interface/v.1.1">

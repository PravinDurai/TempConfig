...
<xsl:stylesheet version="1.0" 
	...
	xmlns:ns0="http://xmlns.vodafone.co.uk/map/til/CustomerManagement/CustomerProfileManagement/CustomerPersonalInformationManagement/CreateCase"
	...
	<xsl:variable name="Severity">
		<xsl:value-of select="$sbResponse/ns0:CreateCaseResponse/ns:ResultStatus/ns:severity"/>
	</xsl:variable>
	<xsl:variable name="ErrorCode">
		<xsl:value-of select="$sbResponse/ns0:CreateCaseResponse/ns:ResultStatus/ns:code"/>
	</xsl:variable>
	<xsl:variable name="Message">
		<xsl:value-of select="$sbResponse/ns0:CreateCaseResponse/ns:ResultStatus/ns:message"/>
	</xsl:variable>
	<xsl:variable name="Component">
		<xsl:choose>
			<xsl:when test="string-length($sbResponse/ns0:CreateCaseResponse/ns:ResultStatus/ns:component) &gt; '0'">
				<xsl:value-of select="$sbResponse/ns0:CreateCaseResponse/ns:ResultStatus/ns:component"/>
			</xsl:when>
			<xsl:otherwise>BW</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="Operation">
		<xsl:value-of select="$sbResponse/ns0:CreateCaseResponse/ns:ResultStatus/ns:operation"/>
	</xsl:variable>
	...
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="$sbResponse/ns0:CreateCaseResponse/ns:ResultStatus/ns:severity='S'">
				<SOAP-ENV:Envelope>
					<SOAP-ENV:Body>
						<v:CreateCaseResponse>
							<xsl:copy-of select="$sbResponse/ns0:CreateCaseResponse/res:ResultStatus"/>
							<v1:caseID>
								<xsl:value-of select="$sbResponse/ns0:CreateCaseResponse/ns0:caseID"/>
							</v1:caseID>
							<v1:traceIdentifier>
								<v2:applicationID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:CreateCase/v1:traceIdentifier/v2:applicationID"/>
								</v2:applicationID>
								<v2:serviceID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:CreateCase/v1:traceIdentifier/v2:serviceID"/>
								</v2:serviceID>
								<v2:serviceInterfaceVersion>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:CreateCase/v1:traceIdentifier/v2:serviceInterfaceVersion"/>
								</v2:serviceInterfaceVersion>
								<v2:timestamp>
									<xsl:value-of select="current-dateTime()"/>
								</v2:timestamp>
								<v2:referenceID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:CreateCase/v1:traceIdentifier/v2:referenceID"/>
								</v2:referenceID>
								<xsl:if test="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:CreateCase/v1:traceIdentifier/v2:correlationID">
								<v2:correlationID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:CreateCase/v1:traceIdentifier/v2:correlationID"/>
								</v2:correlationID>
								</xsl:if>
								<v2:identityID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:CreateCase/v1:traceIdentifier/v2:identityID"/>
								</v2:identityID>
							</v1:traceIdentifier>
						</v:CreateCaseResponse>
					</SOAP-ENV:Body>

	...
	<xsl:template match="/">
		<details>
			...
			<BusinessKey>
				<Name>MSISDN</Name>
				<Value>
					<xsl:value-of select="$cnRequest/SOAP-ENV:Envelope/SOAP-ENV:Body/v:AddPrepayService/v1:msisdn"/>
				</Value>
			</BusinessKey>
			<RequestPayload>
				<set:AddPrepayServiceRequest>
					<set:msisdn>
						<xsl:value-of select="$cnRequest/SOAP-ENV:Envelope/SOAP-ENV:Body/v:AddPrepayService/v1:msisdn"/>
					</set:msisdn>
                <xsl:for-each select="$cnRequest/SOAP-ENV:Envelope/SOAP-ENV:Body/v:AddPrepayService/v1:Service">
					<srvc:Service>
			...
			<TargetOperation>AddPrepayService</TargetOperation>
		</details>
	</xsl:template>
</xsl:stylesheet>
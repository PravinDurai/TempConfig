	...
	<xsl:template match="/">
		<details>
			...
			<BusinessKey>
				<Name>MSISDN</Name>
				<Value>
					<xsl:value-of select="$cnRequest/SOAP-ENV:Envelope/SOAP-ENV:Body/v:DataNotificationOptIn/v1:MSISDN"/>
				</Value>
			</BusinessKey>
			<RequestPayload>
				<set:DataNotificationOptInRequest>
					<set:MSISDN>
						<xsl:value-of select="$cnRequest/SOAP-ENV:Envelope/SOAP-ENV:Body/v:DataNotificationOptIn/v1:MSISDN"/>
					</set:MSISDN>
					<xsl:if test="$cnRequest/SOAP-ENV:Envelope/SOAP-ENV:Body/v:DataNotificationOptIn/v1:NotificationType">
						<set:NotificationType>
							<xsl:value-of select="$cnRequest/SOAP-ENV:Envelope/SOAP-ENV:Body/v:DataNotificationOptIn/v1:NotificationType"/>
						</set:NotificationType>
					</xsl:if>
				</set:DataNotificationOptInRequest>
			</RequestPayload>
			<TargetOperation>DataNotificationOptIn</TargetOperation>
		</details>
	</xsl:template>
</xsl:stylesheet>
...
<xsl:stylesheet version="1.0" 
	...
	<xsl:variable name="IdentityID"><xsl:value-of select="$context/v:SelfcareSubscription/v1:traceIdentifier/v2:identityID"/></xsl:variable>
	...
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="$IdentityID='VFUKTIL'">
				<xsl:choose>
					<xsl:when test="$sbResponse/er-response[@id='100001']">
						<v:SelfcareSubscriptionServiceFault  xmlns:ns1 = "http://xmlns.vodafone.co.uk/RIG/common/Fault">
            ...
            , error@system-id - <xsl:value-of select="$sbResponse/er-response/payload/error/attribute::system-id"/>
						...
						</v:SelfcareSubscriptionServiceFault>
					</xsl:when>
					<xsl:otherwise>
						<v:SelfcareSubscriptionResponse>
							<v1:er-response>
                              ...
                              <xsl:for-each select="$sbResponse/er-response/payload/selfcare-subscriptions">
								<v1:selfcare-subscriptions>
							...
							<v1:traceIdentifier>
								<v2:applicationID>
									<xsl:value-of select="$context/v:SelfcareSubscription/v1:traceIdentifier/v2:applicationID"/>
								</v2:applicationID>
								<v2:serviceID>
									<xsl:value-of select="$context/v:SelfcareSubscription/v1:traceIdentifier/v2:serviceID"/>
								</v2:serviceID>
								<v2:serviceInterfaceVersion>
									<xsl:value-of select="$context/v:SelfcareSubscription/v1:traceIdentifier/v2:serviceInterfaceVersion"/>
								</v2:serviceInterfaceVersion>
								<v2:timestamp>
									<xsl:value-of select="current-dateTime()"/>
								</v2:timestamp>
								<v2:referenceID>
									<xsl:value-of select="$context/v:SelfcareSubscription/v1:traceIdentifier/v2:referenceID"/>
								</v2:referenceID>
								<v2:correlationID>
									<xsl:value-of select="$context/v:SelfcareSubscription/v1:traceIdentifier/v2:correlationID"/>
								</v2:correlationID>
								<v2:identityID>
									<xsl:value-of select="$context/v:SelfcareSubscription/v1:traceIdentifier/v2:identityID"/>
								</v2:identityID>
							</v1:traceIdentifier>
						</v:SelfcareSubscriptionResponse>
					</xsl:otherwise>

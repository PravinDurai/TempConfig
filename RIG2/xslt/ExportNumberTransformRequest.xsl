	...
	<xsl:template match="/">
		<details>
			...
			<BusinessKey>
				<Name>MSISDN</Name>
				<Value>
					<xsl:value-of select="$cnRequest/SOAP-ENV:Envelope/SOAP-ENV:Body/v:ExportNumber/v1:CTN"/>
				</Value>
			</BusinessKey>
			<RequestPayload>
			<set:ExportNumberRequest>
					<set:CTN><xsl:value-of select="$cnRequest/SOAP-ENV:Envelope/SOAP-ENV:Body/v:ExportNumber/v1:CTN"/></set:CTN>
					<set:target><xsl:value-of select="$cnRequest/SOAP-ENV:Envelope/SOAP-ENV:Body/v:ExportNumber/v1:target"/></set:target>
			</set:ExportNumberRequest>
			</RequestPayload>
			<TargetOperation>ExportNumber</TargetOperation>
		</details>
	</xsl:template>
</xsl:stylesheet>
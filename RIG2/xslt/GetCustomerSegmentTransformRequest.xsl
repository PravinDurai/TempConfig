	...
	<xsl:template match="/">
		<details>
			...
			<BusinessKey>
				<Name>MSISDN</Name>
				<Value>
					<xsl:value-of select="$cnRequest/SOAP-ENV:Envelope/SOAP-ENV:Body/v:GetCustomerSegment/v1:CTN"/>
				</Value>
			</BusinessKey>
			<RequestPayload>
				<set:GetCustomerSegmentRequest>
					<set:CTN>
						<xsl:value-of select="$cnRequest/SOAP-ENV:Envelope/SOAP-ENV:Body/v:GetCustomerSegment/v1:CTN"/>
					</set:CTN>
				</set:GetCustomerSegmentRequest>
			</RequestPayload>
			<TargetOperation>GetCustomerSegment</TargetOperation>
		</details>
	</xsl:template>
</xsl:stylesheet>
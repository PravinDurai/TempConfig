...
xmlns:v1="http://www.vodafone.com/vf/manageCustomerAccount/messages/v.1.1"    xmlns:set = "http://xmlns.vodafone.co.uk/map/til/Billing/BalanceManagement/AccountTypeManagement/Migrate" xmlns:res="http://xmlns.vodafone.co.uk/map/til/Common/ResultStatus" xmlns:v2="http://www.vodafone.com/vf/core_common/types/v.0.3/" xmlns:ns="http://xmlns.vodafone.co.uk/map/til/Common/ResultStatus">
	...
	<xsl:variable name="Severity">
		<xsl:value-of select="$sbResponse/set:MigrateResponse/ns:ResultStatus/ns:severity"/>
	</xsl:variable>
	<xsl:variable name="ErrorCode">
		<xsl:value-of select="$sbResponse/set:MigrateResponse/ns:ResultStatus/ns:code"/>
	</xsl:variable>
	<xsl:variable name="Message">
		<xsl:value-of select="$sbResponse/set:MigrateResponse/ns:ResultStatus/ns:message"/>
	</xsl:variable>
	<xsl:variable name="Component">
		<xsl:choose>
			<xsl:when test="string-length($sbResponse/set:MigrateResponse/ns:ResultStatus/ns:component) &gt; '0'">
				<xsl:value-of select="$sbResponse/set:MigrateResponse/ns:ResultStatus/ns:component"/>
			</xsl:when>
			<xsl:otherwise>BW</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="Operation">
		<xsl:value-of select="$sbResponse/set:MigrateResponse/ns:ResultStatus/ns:operation"/>
	</xsl:variable>
	...
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="$sbResponse/set:MigrateResponse/ns:ResultStatus/ns:severity='S'">
				<SOAP-ENV:Envelope>
					<SOAP-ENV:Body>
						<v:MigrateResponse>
							<xsl:copy-of select="$sbResponse/set:MigrateResponse/ns:ResultStatus"/>
							<v1:traceIdentifier>
								<v2:applicationID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:Migrate/v1:traceIdentifier/v2:applicationID"/>
								</v2:applicationID>
								<v2:serviceID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:Migrate/v1:traceIdentifier/v2:serviceID"/>
								</v2:serviceID>
								<v2:serviceInterfaceVersion>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:Migrate/v1:traceIdentifier/v2:serviceInterfaceVersion"/>
								</v2:serviceInterfaceVersion>
								<v2:timestamp>
									<xsl:value-of select="current-dateTime()"/>
								</v2:timestamp>
								<v2:referenceID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:Migrate/v1:traceIdentifier/v2:referenceID"/>
								</v2:referenceID>
								<xsl:if test="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:Migrate/v1:traceIdentifier/v2:correlationID">
									<v2:correlationID>
										<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:Migrate/v1:traceIdentifier/v2:correlationID"/>
									</v2:correlationID>
								</xsl:if>
								<v2:identityID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:Migrate/v1:traceIdentifier/v2:identityID"/>
								</v2:identityID>
							</v1:traceIdentifier>
						</v:MigrateResponse>
					</SOAP-ENV:Body>

...
xmlns:chec="http://xmlns.vodafone.co.uk/map/til/CustomerManagement/CustomerIncentivesManagement/IncentivesDefinition/CheckRetentionEligibility" xmlns:res="http://xmlns.vodafone.co.uk/map/til/Common/ResultStatus" 
...
<xsl:copy-of select="codecs:base64ToText($JMSResponse/h:body)"/>
   ...
   <xsl:variable name="sbResponse">
	...
	<xsl:template match="chec:CheckRetentionEligibilityResponse/chec:RetentionEligibilityList/v1:ContractExpiryDate"/>
	<xsl:template match="chec:CheckRetentionEligibilityResponse/chec:RetentionEligibilityList/v1:EarlyStartDate"/>
    <xsl:template match="chec:CheckRetentionEligibilityResponse/chec:RetentionEligibilityList/v2:ApplicationObjectKey"/>
	<xsl:template match="chec:CheckRetentionEligibilityResponse/chec:RetentionEligibilityList/v2:ApplicationObjectKey/v2:ContextID"/>	
    <xsl:template match="chec:CheckRetentionEligibilityResponse/chec:RetentionEligibilityList/v2:ApplicationObjectKey/v2:ID"/>		
</xsl:stylesheet>
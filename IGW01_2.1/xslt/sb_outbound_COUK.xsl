...
<xsl:variable name="formattedCurrentDateTime">
        <xsl:value-of select="format-dateTime(current-dateTime(),&quot;[Y]-[M]-[D]T[H01]:[m01]:[s01].[f001]Z&quot;)"/>
</xsl:variable>
...
<m:mapping-result>
	    ...
	    <c:entry key="asg:httpResponse">
	     <h:response>
		   ...
		   <h:override-header name="date"><xsl:value-of select="$formattedCurrentDateTime"/></h:override-header>
  	       
  	        <m:payload-text>
			<xsl:value-of select="codecs:base64ToText($jsonResponse)"/>  
	        </m:payload-text>

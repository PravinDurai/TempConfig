...
<xsl:stylesheet version="1.0" xmlns:ns1="http://www.vodafone.com/rig/internal/validateheader/v1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:head="http://xmlns.vodafone.co.uk/map/til/wsdl/Common/Header" xmlns:log="http://xmlns.vodafone.co.uk/map/til/Common/CommonObjects/Logon" xmlns:v="http://www.vodafone.com/vf/crmInteractionHandling/interface/v.1.0" xmlns:v1="http://www.vodafone.com/vf/crmInteractionHandling/messages/v.1.0"  xmlns:set = "http://xmlns.vodafone.co.uk/map/til/CustomerManagement/CustomerProfileManagement/CustomerPersonalInformationManagement/CreateInteraction" xmlns:ns4="http://xmlns.vodafone.co.uk/map/til/Common/CommonObjects/Interaction" xmlns:res="http://xmlns.vodafone.co.uk/map/til/Common/ResultStatus" xmlns:v2="http://www.vodafone.com/vf/core_common/types/v.0.3/" xmlns:ns="http://xmlns.vodafone.co.uk/map/til/Common/ResultStatus">
	...
	<xsl:variable name="Severity">
		<xsl:value-of select="$sbResponse/set:CreateInteractionResponse/ns:ResultStatus/ns:severity"/>
	</xsl:variable>
	<xsl:variable name="ErrorCode">
		<xsl:value-of select="$sbResponse/set:CreateInteractionResponse/ns:ResultStatus/ns:code"/>
	</xsl:variable>
	<xsl:variable name="Message">
		<xsl:value-of select="$sbResponse/set:CreateInteractionResponse/ns:ResultStatus/ns:message"/>
	</xsl:variable>
	<xsl:variable name="Component">
		<xsl:choose>
			<xsl:when test="string-length($sbResponse/set:CreateInteractionResponse/ns:ResultStatus/ns:component) &gt; '0'">
				<xsl:value-of select="$sbResponse/set:CreateInteractionResponse/ns:ResultStatus/ns:component"/>
			</xsl:when>
			<xsl:otherwise>BW</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="Operation">
		<xsl:value-of select="$sbResponse/set:CreateInteractionResponse/ns:ResultStatus/ns:operation"/>
	</xsl:variable>
	...
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="$sbResponse/set:CreateInteractionResponse/ns:ResultStatus/ns:severity='S'">
				<SOAP-ENV:Envelope>
					<SOAP-ENV:Header>
					...
					<SOAP-ENV:Body>
						<v:CreateInteractionResponse>
                        <xsl:copy-of select="$sbResponse/set:CreateInteractionResponse/ns:ResultStatus"/>
                        <xsl:copy-of select="$sbResponse/set:CreateInteractionResponse/ns4:Interaction"/>
							<v1:traceIdentifier>
								<v2:applicationID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:CreateInteraction/v1:traceIdentifier/v2:applicationID"/>
								</v2:applicationID>
								<v2:serviceID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:CreateInteraction/v1:traceIdentifier/v2:serviceID"/>
								</v2:serviceID>
								<v2:serviceInterfaceVersion>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:CreateInteraction/v1:traceIdentifier/v2:serviceInterfaceVersion"/>
								</v2:serviceInterfaceVersion>
								<v2:timestamp>
									<xsl:value-of select="current-dateTime()"/>
								</v2:timestamp>
								<v2:referenceID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:CreateInteraction/v1:traceIdentifier/v2:referenceID"/>
								</v2:referenceID>
                                <xsl:if test="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:CreateInteraction/v1:traceIdentifier/v2:correlationID">
								<v2:correlationID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:CreateInteraction/v1:traceIdentifier/v2:correlationID"/>
								</v2:correlationID>
								</xsl:if>
								<v2:identityID>
									<xsl:value-of select="$context/SOAP-ENV:Envelope/SOAP-ENV:Body/v:CreateInteraction/v1:traceIdentifier/v2:identityID"/>
								</v2:identityID>
							</v1:traceIdentifier>
						</v:CreateInteractionResponse>
					</SOAP-ENV:Body>
				...
				<SOAP-ENV:Envelope xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/">
					<SOAP-ENV:Body>
						<SOAP-ENV:Fault>
							...
							<detail>
								<v:CreateInteractionserviceException>

<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
   ...
   exclude-result-prefixes="xs v1 ns xsi SOAP-ENV">
	...
	<xsl:variable name="Target">
		<xsl:choose>
			<xsl:when test="$CountryCode='UK'">
				<xsl:value-of select="&quot;CESB&quot;"/>
			</xsl:when>
			<xsl:when test="$CountryCode!='UK' and $RoutingServiceClass='Fixed'">
				<xsl:value-of select="&quot;WMMEDIATOR&quot;"/>
			</xsl:when>
			<xsl:otherwise/>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:variable name="RoutingKey">
		<xsl:choose>
			<xsl:when test="$Target='CESB'">
				<RoutingKey>CESB</RoutingKey>
			</xsl:when>
			<xsl:when test="$Target='WMMEDIATOR'">
				<RoutingKey>WMMEDIATOR</RoutingKey> 
			</xsl:when>
